...
// Imported from their individual source modules rather than the
// `@alienkitty/alien.js/three` barrel, which transitively pulls in the
// OimoPhysics wrapper (an uninstalled `oimophysics` dependency) and breaks the
// production build.
import { BloomCompositeMaterial } from '@alienkitty/alien.js/src/three/materials/BloomCompositeMaterial.js';
import { BlurMaterial } from '@alienkitty/alien.js/src/three/materials/BlurMaterial.js';
import { LuminosityMaterial } from '@alienkitty/alien.js/src/three/materials/LuminosityMaterial.js';
import { SMAABlendMaterial } from '@alienkitty/alien.js/src/three/materials/SMAABlendMaterial.js';
import { SMAAEdgesMaterial } from '@alienkitty/alien.js/src/three/materials/SMAAEdgesMaterial.js';
import { SMAAWeightsMaterial } from '@alienkitty/alien.js/src/three/materials/SMAAWeightsMaterial.js';
import { UnrealBloomBlurMaterial } from '@alienkitty/alien.js/src/three/materials/UnrealBloomBlurMaterial.js';
...
/**
 * Multi-pass render pipeline for the Mars example.
 *
 * Ported verbatim (numerically) from the static `RenderManager` in
 * `examples/mars/src/controllers/world/RenderManager.js` to an instance so it
 * can be created and disposed alongside the R3F canvas. The renderer, scene,
 * camera and view are supplied by the React scene; the shared `world` provides
 * the fullscreen triangle and global uniforms.
 */
export class RenderManager {
    ...
    init(renderer, scene, camera, view) {
    ...
    destroy() {
        const targets = [
            ...
            this.renderTargetBloomA,
            this.renderTargetBloomB,
            this.renderTargetBlurA,
            this.renderTargetBlurB,
            this.renderTargetBright,
            ...(this.renderTargetsHorizontal || []),
            ...(this.renderTargetsVertical || [])
        ];

        targets.forEach(target => target && target.dispose());
        ...
        const materials = [
            ...
            this.vBlurMaterial,
            this.vlMaterial,
            this.luminosityMaterial,
            this.bloomCompositeMaterial,
            this.sceneCompositeMaterial,
