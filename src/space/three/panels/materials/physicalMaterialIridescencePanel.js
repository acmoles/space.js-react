/**
 * @author Space.js React
 *
 * Iridescence sub-panel for MeshPhysicalMaterial.
 *
 * Ported from the reference `PhysicalMaterialIridescencePanel` class.
 */

import { subPanel } from '../subPanel.js';
import { mapPanelItems } from '../textures/mapPanel.js';

/**
 * @param {import('three').Mesh} mesh
 * @param {object}               ui
 * @returns {object[]} Panel item descriptors.
 */
export function physicalMaterialIridescencePanelItems(mesh, ui) {
    const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    const material = materials[0];

    return [
        {
            type: 'content',
            callback: (value, item) => {
                item.setContent(subPanel(mapPanelItems(mesh, ui, 'iridescenceMap')));
            }
        },
        {
            type: 'divider'
        },
        {
            type: 'slider',
            name: 'Int',
            min: 0,
            max: 1,
            step: 0.01,
            value: material.iridescence,
            callback: value => {
                materials.forEach(material => material.iridescence = value);
            }
        },
        {
            type: 'slider',
            name: 'IOR',
            min: 1,
            max: 2.333,
            step: 0.01,
            value: material.iridescenceIOR,
            callback: value => {
                materials.forEach(material => material.iridescenceIOR = value);
            }
        }
    ];
}
